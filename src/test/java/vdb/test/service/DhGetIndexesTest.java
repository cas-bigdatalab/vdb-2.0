...
public class DhGetIndexesTest
{
	...
	// 服务功能：索引数据收割接口
	@Test
	public void testDhGetIndexes()
	{
		// 请求方式错误测试
		{
			HttpClient client = new HttpClient();
			PostMethod method = new PostMethod(SERVICE_URL);
			method.setParameter("verb", "dhGetIndexes");
		...
		// 传递错误参数的测试
		{
			HttpClient client = new HttpClient();
			PostMethod method = new PostMethod(SERVICE_URL);
			method.setParameter("verb", "dhGetIndexes");// 设置服务的名称
			method.setParameter("uri", "localhost");// 设置数据集的URI，这里传递进去的是错误的URI，程序在后台会出现异常，并定位到500错误
			String returnStr = ServiceUtil.getReturnString(client, method);
			if (returnStr.equals(""))
				Assert.fail("执行getReturnString()方法时出现异常，请确保方法的正确性");
			else
				...
				else
				{
					...
					else
					{
						try
						{
							...
							Element itemElement = (Element) XPath
									.selectSingleNode(rootElement,
							...
							{
								Assert.assertEquals("4283", totalSizeElement
										.getTextTrim());
								Assert.assertEquals(
										"localhost.gem.indicatorbasicinfo/2",
										nextIndexElement.getTextTrim());
								Assert.assertEquals(
										"localhost.gem.indicatorbasicinfo/0",
										itemElement.getTextTrim());
							}
