 ...
 */function dhtmlXMenuObject(baseId, skin) {
	...
	if (!this.extendedModule) {
		...
		var extMethods = new Array("setItemEnabled", "setItemDisabled",
				"isItemEnabled", "_changeItemState", "getItemText",
				"setItemText", "loadFromHTML", "hideItem", "showItem",
				"isItemHidden", "_changeItemVisible", "setUserData",
				"getUserData", "setOpenMode", "setWebModeTimeout",
				...
				"_checkArrowsState", "_addUpArrow", "_addDownArrow",
				"_removeUpArrow", "_removeDownArrow", "_isArrowExists",
				"_doScrollUp", "_doScrollDown", "_countPolygonItems",
				"setOverflowHeight", "_getRadioImgObj", "_setRadioState",
				"_radioOnClickHandler", "getRadioChecked", "setRadioChecked",
				"addRadioButton", "_getCheckboxState", "_setCheckboxState",
				"_readLevel", "_updateCheckboxImage",
				"_checkboxOnClickHandler", "setCheckboxState",
	...
	this._arrowFFFix = (_isIE ? (document.compatMode == "BackCompat" ? 0 : -4)
			: -4);
	this.setSkin = function(skin) {
		...
		for ( var a in this.idPull) {
			this.idPull[a].className = String(this.idPull[a].className)
					.replace(oldSkin, this.skin)
		}
	};
	...
	this._getSubItemToDeselectByPolygon = function(polygon) {
		...
		for ( var q = 0; q < this._selectedSubItems.length; q++) {
			if (this._selectedSubItems[q][1] == polygon) {
				...
				m = m
						.concat(this
								._getSubItemToDeselectByPolygon(this._selectedSubItems[q][0]));
				var t = true;
	...
	this._showPolygon = function(id, openType) {
		...
		if (itemCount == 0)
			return;
		var FC = "polygon_" + id;
		if ((this.idPull[FC] != null) && (this.idPull[id] != null)) {
			...
			;
			this.idPull[FC].style.visibility = "hidden";
			this.idPull[FC].style.left = "0px";
			this.idPull[FC].style.top = "0px";
			this.idPull[FC].style.display = "";
			this.idPull[FC].style.zIndex = this.zInd;
			if (this.limit > 0) {
				if (this.limit < itemCount) {
					this.idPull[FC].style.height = this.idPull[FC].childNodes[0].offsetHeight
							* this.limit + "px";
					this.idPull[FC].scrollTop = 0
				} else {
					this.idPull[FC].style.height = ""
				}
			}
			;
			...
			;
			var srcX = (this.idPull[id].tagName != null ? ej(this.idPull[id])
					: this.idPull[id][0]);
			var srcY = (this.idPull[id].tagName != null ? dg(this.idPull[id])
					: this.idPull[id][1]);
			var srcW = (this.idPull[id].tagName != null ? this.idPull[id].offsetWidth
					: 0);
			var srcH = (this.idPull[id].tagName != null ? this.idPull[id].offsetHeight
					+ arrUpH + arrDownH
					: 0);
			var x = 0;
			var y = 0;
			var w = this.idPull[FC].offsetWidth;
			var h = this.idPull[FC].offsetHeight;
			if (openType == "bottom") {
				if (this._rtl) {
					...
					if (this.js == "right") {
						...
						x = srcX
								- 1
								+ (openType == this.dirTopLevel ? this._topLevelRightMargin
										: 0)
					}
				}
				;
				y = srcY - 1 + srcH - arrUpH - arrDownH
			...
			;
			if (openType == "left") {
				x = srcX - this.idPull[FC].offsetWidth + 2;
				y = srcY + 2
			}
			;
			if (openType == "top") {
			...
			if (y + h > my && this.menuY2 != null) {
				...
				if (this.itemPull[id] != null && !this.context) {
					if (this.itemPull[id]["parent"] == this.idPrefix
							+ this.topId)
						y = y - this.base.offsetHeight
				}
			}
			;
			this.idPull[FC].style.left = x + "px";
			this.idPull[FC].style.top = y + arrUpH + "px";
			if ((this.sxDacProc != null)
					&& (this.idPull["sxDac_" + id] != null)) {
				this.idPull["sxDac_" + id]._show()
			} else {
				this.idPull[FC].style.visibility = "";
				if (this.limit > 0 && this.limit < itemCount) {
				...
				if (this._isIE6) {
					...
					;
					this.idPull[pIdIE6].style.left = this.idPull[FC].style.left;
					this.idPull[pIdIE6].style.top = this.idPull[FC].style.top;
					this.idPull[pIdIE6].style.width = this.idPull[FC].offsetWidth
							+ "px";
					this.idPull[pIdIE6].style.height = this.idPull[FC].offsetHeight
							+ "px";
					this.idPull[pIdIE6].style.zIndex = this.idPull[FC].style.zIndex - 1;
	...
	this._redistribSubLevelSelection = function(id, parentId) {
		...
		if (this.itemPull[id]["state"] == "enabled") {
			...
			if (this.itemPull[id]["complex"] && this.dLoad
				...
				;
				var bA = new ag(this._xmlParser, window);
				this.itemPull[id]["loaded"] = "get";
				this.callEvent("onXLS", []);
				bA.loadXML(this.dLoadUrl + this.dLoadSign
						+ "action=loadMenu&parentId="
						+ id.replace(this.idPrefix, "") + "&etc="
						+ new Date().getTime())
			}
			;
	...
	this._doOnClick = function(id, type, casState) {
		...
		if (this.mR("onClick")) {
			...
			if ((type.charAt(1) == "d")
					|| (this.menuMode == "win" && type.charAt(2) == "t"))
				return;
			this._clearAndHide();
			if (this._isContextMenuVisible() && this.contextAutoHide)
				this._hideContextMenu()
			...
			if (typeof (menu[q]) == "object") {
				if (menu[q].length == 5) {
					if (typeof (menu[q][0]) != "object") {
						if ((menu[q][0].replace(this.idPrefix, "") == node)
								&& (q == 0)) {
							m = menu
						}
					}
		...
		for ( var q = 0; q < main_self._selectedSubItems.length; q++) {
			...
			if (main_self.idPull[id] != null) {
				if (main_self.itemPull[id]["state"] == "enabled") {
					if (main_self.idPull[id].className == "dhtmlxMenu_"
							+ main_self.skin + "_SubLevelArea_Item_Selected") {
						main_self.idPull[id].className = "dhtmlxMenu_"
								+ main_self.skin + "_SubLevelArea_Item_Normal"
					}
					;
					if (main_self.idPull[id].className == "dhtmlxMenu_"
							+ main_self.skin + "_TopLevel_Item_Selected") {
						if (main_self.itemPull[id]["cssNormal"] != null) {
							main_self.idPull[id].className = main_self.itemPull[id]["cssNormal"]
						} else {
							main_self.idPull[id].className = "dhtmlxMenu_"
									+ main_self.skin + "_TopLevel_Item_Normal"
						}
					}
				}
			}
	...
	this.loadXMLString = function(RA, onLoadFunction) {
		if (onLoadFunction)
			this._doOnLoad = function() {
				onLoadFunction()
			};
		this._xmlLoader.loadXMLString(RA)
	};
	this._buildMenu = function(t, parentId) {
		...
		for ( var q = 0; q < t.childNodes.length; q++) {
			if (t.childNodes[q].tagName == this.itemTagName) {
				...
				var item = {};
				item["id"] = this.idPrefix
						+ (r.getAttribute("id") || this._genStr(24));
				...
				;
				item["state"] = (r.getAttribute("enabled") != null
						|| r.getAttribute("disabled") != null ? (r
						.getAttribute("enabled") == "false"
						|| r.getAttribute("disabled") == "true" ? "disabled"
						: "enabled") : "enabled");
				item["parent"] = (parentId != null ? parentId : this.idPrefix
						+ this.topId);
				item["complex"] = (this.dLoad ? (r.getAttribute("complex") != null ? true
						: false)
				...
				for ( var w = 0; w < r.childNodes.length; w++) {
					...
					if (tagNm == this.userDataTagName) {
						...
						if (d.getAttribute("name") != null) {
							this.userData[item["id"] + "_"
									+ d.getAttribute("name")] = (d.firstChild.nodeValue != null ? d.firstChild.nodeValue
									: "")
						}
					}
					;
					...
					if (tagNm == this.itemHrefTagName && item["type"] == "item") {
						...
						if (r.childNodes[w].getAttribute("target") != null) {
							item["href_target"] = r.childNodes[w]
									.getAttribute("target")
						}
					}
				}
	...
	this._xmlParser = function() {
		if (main_self.dLoad) {
			var t = this.cR("menu");
			parentId = (t.getAttribute("parentId") != null ? t
			...
			if (parentId == null) {
				...
				main_self._addSubMenuPolygon(main_self.idPrefix + parentId,
						main_self.idPrefix + parentId);
				if (main_self.menuSelected == main_self.idPrefix + parentId) {
					var FC = main_self.idPrefix + parentId;
					var isTop = main_self.itemPull[main_self.idPrefix
							+ parentId]["parent"] == main_self.idPrefix
							+ main_self.topId;
					var gR = ((isTop && (!main_self.context)) ? main_self.dirTopLevel
							: main_self.dirSubLevel);
					var isShow = false;
					if (isTop && main_self.menuModeTopLevelTimeout
							&& main_self.menuMode == "web"
							&& !main_self.context) {
						var item = main_self.idPull[main_self.idPrefix
								+ parentId];
						if (item._mouseOver == true) {
							var amY = main_self.menuModeTopLevelTimeoutTime
									- (new Date().getTime() - item._dynLoadTM);
							if (amY > 1) {
								item._menuOpenTM = window.setTimeout(
										function() {
											main_self._showPolygon(FC, gR)
										}, amY);
								isShow = true
							}
						}
				...
				if (main_self.loaderIcon == true) {
					main_self._updateLoaderIcon(main_self.idPrefix + parentId,
							false)
				}
			}
			;
	...
	this._xmlLoader = new ag(this._xmlParser, window);
	this._showSubLevelItem = function(id, back) {
		...
		if (document.getElementById(this.idPrefix + id) != null) {
			document.getElementById(this.idPrefix + id).style.display = (back ? ""
					: "none")
		}
	};
	...
	dE(document.body, "click", function(e) {
	...
	dE(document.body, "contextmenu", function(e) {
		if (main_self._unloaded)
			return;
		e = e || event;
...
dhtmlXMenuObject.prototype._countVisiblePolygonItems = function(id) {
	var count = 0;
	for ( var a in this.itemPull) {
		var asv = this.itemPull[a]["parent"];
		var tp = this.itemPull[a]["type"];
		if (this.idPull[a] != null) {
			if (asv == id
					&& (tp == "item" || tp == "radio" || tp == "checkbox")
...
dhtmlXMenuObject.prototype._updateItemComplexState = function(id, state, over) {
	...
	if (this.itemPull[id]["complex"]) {
		...
		if (this.dLoad && (this.itemPull[id]["loaded"] == "get")
			...
			var genName = "arrow"
					+ (this._rtl ? "r" : "l")
					+ "_"
					+ (this.itemPull[id]["state"] == "enabled" ? (over ? "over"
							: "en") : "dis");
			arrowObj.className = "dhtmlxMenu_SubLevelArea_Item_Arrow "
					+ genName
		}
		;
		return
	...
	if ((!this.itemPull[id]["complex"]) && (arrowObj != null)) {
		...
		if (this.itemPull[id]["hotkey_backup"] != null && this.setHotKey) {
			this.setHotKey(id.replace(this.idPrefix, ""),
					this.itemPull[id]["hotkey_backup"])
		}
	}
};
dhtmlXMenuObject.prototype._getItemLevelType = function(id) {
	return (this.itemPull[this.idPrefix + id]["parent"] == this.idPrefix
	...
	if (this.js == "left") {
		for ( var q = 0; q < this.base.childNodes.length; q++) {
			if (this.base.childNodes[q].tagName == "DIV") {
				if (String(this.base.childNodes[q].className).search(
						"TopLevel_Text") == -1) {
					if (!this._rtl) {
						this.base.childNodes[q].style.right = "";
						this.base.childNodes[q].style.left = "";
						//this.base.childNodes[q].style.float = "left";						
						//this.base.childNodes[q].style.position = "inherit";
						this.base.childNodes[q].style.width = "auto";
						if(q>=this.base.childNodes.length-2)
						{
							this.base.childNodes[q].style.float = "right";
							//this.base.childNodes[q].style.margin = "0px auto";
						}
						else
							this.base.childNodes[q].style.float = "left";	
					} else {
					...
					if (this.skin == "dhx_skyblue"
							&& String(this.base.childNodes[q].className)
									.search("Separator") == -1)
						w += 2
				}
			}
...
dhtmlXMenuObject.prototype._redistribTopLevelSelection = function(id, parent) {
	...
	for ( var q = 0; q < i.length; q++) {
		...
		;
		if ((this.idPull[i[q]] != null) && (i[q] != id)) {
			this.idPull[i[q]].className = this.idPull[i[q]].className.replace(
					/Selected/g, "Normal")
		}
	}
	;
	if (this.itemPull[this.idPrefix + id]["state"] == "enabled") {
		...
		this.menuSelected = (this.menuMode == "win" ? (this.menuSelected != -1 ? id
				: this.menuSelected)
				: id);
		if ((this.itemPull[this.idPrefix + id]["complex"])
				&& (this.menuSelected != -1)) {
...
dhtmlXMenuObject.prototype._renderToplevelItem = function(id, pos) {
	...
	m.id = id;
	if (this.itemPull[id]["state"] == "enabled"
			&& this.itemPull[id]["cssNormal"] != null) {
		m.className = this.itemPull[id]["cssNormal"]
	} else {
		m.className = "dhtmlxMenu_"
				+ this.skin
				+ "_TopLevel_Item_"
				+ (this.itemPull[id]["state"] == "enabled" ? "Normal"
						: "Disabled")
	}
	;
	if (this.itemPull[id]["title"].indexOf('|') >= 0) {
		var tempLink = this.itemPull[id]["title"].split('|');
		m.innerHTML = "<a href='" + tempLink[1]
				+ "' onclick=\"document.location.href='" + tempLink[1]
				+ "'\" target='_blank'>" + tempLink[0] + "</a>";
	} else {
	...
	;
	if ((this.itemPull[id]["imgen"] != "")
			|| (this.itemPull[id]["imgdis"] != "")) {
		var imgTop = this.itemPull[id][(this.itemPull[id]["state"] == "enabled") ? "imgen"
				: "imgdis"];
		if (imgTop) {
			var imgTop = "<img id='image_" + id + "' src='" + this.imagePath
					+ imgTop + "' class='dhtmlxMenu_TopLevel_Item_Icon_"
	...
	this.idPull[m.id] = m;
	if (this.itemPull[id]["complex"] && (!this.dLoad)) {
		this._addSubMenuPolygon(this.itemPull[id]["id"],
				this.itemPull[id]["id"])
	}
	;
	m.onmouseover = function() {
		...
		for ( var q = 0; q < i.length; q++) {
			...
			if ((main_self.idPull[i[q]] != null) && (i[q] != this.id)) {
				if (main_self.itemPull[i[q]]["cssNormal"] != null) {
					...
					main_self.idPull[i[q]].className = main_self.idPull[i[q]].className
		...
		if (main_self.itemPull[this.id]["state"] == "enabled") {
			this.className = "dhtmlxMenu_" + main_self.skin
					+ "_TopLevel_Item_Selected";
			...
			main_self.menuSelected = (main_self.menuMode == "win" ? (main_self.menuSelected != -1 ? this.id
					...
					: this.id);
			if (main_self.dLoad
					&& (main_self.itemPull[this.id]["loaded"] == "no")) {
				if (main_self.menuModeTopLevelTimeout
						&& main_self.menuMode == "web" && !main_self.context) {
					this._mouseOver = true;
					this._dynLoadTM = new Date().getTime()
				}
				;
				var bA = new ag(main_self._xmlParser, window);
				main_self.itemPull[this.id]["loaded"] = "get";
				main_self.callEvent("onXLS", []);
				bA.loadXML(main_self.dLoadUrl + main_self.dLoadSign
						+ "action=loadMenu&parentId="
						+ this.id.replace(main_self.idPrefix, "") + "&etc="
						+ new Date().getTime())
			}
			;
			if ((!main_self.dLoad)
				...
				if ((main_self.itemPull[this.id]["complex"])
					...
					if (main_self.menuModeTopLevelTimeout
						...
						this._menuOpenTM = window.setTimeout( function() {
							main_self._showPolygon(showItemId,
									main_self.dirTopLevel)
	...
	m.onmouseout = function() {
		if (!((main_self.itemPull[this.id]["complex"]) && (main_self.menuSelected != -1))
			...
			if (main_self.itemPull[this.id]["cssNormal"] != null) {
				...
				m.className = "dhtmlxMenu_" + main_self.skin
	...
	m.onclick = function(e) {
		...
		;
		if (main_self.menuMode != "web"
				&& main_self.itemPull[this.id]["state"] == "disabled") {
			return
		}
		;
		e = e || event;
		...
		main_self._doOnClick(this.id.replace(main_self.idPrefix, ""), tc + td
				+ "t", cas);
		return false
	}
};
...
dhtmlXMenuObject.prototype._updateItemImage = function(id, levelType) {
	...
	if (this.itemPull[this.idPrefix + id]["type"] == "radio") {
		var imgSrc = this.itemPull[this.idPrefix + id][(this.itemPull[this.idPrefix
				+ id]["state"] == "enabled" ? "imgen" : "imgdis")]
	} else {
		var imgSrc = this.itemPull[this.idPrefix + id][(this.itemPull[this.idPrefix
				+ id]["state"] == "enabled" ? "imgen" : "imgdis")]
	}
	...
	if (imgSrc.length > 0) {
		if (imgObj != null) {
			...
			if (tp == "checkbox" || tp == "radio") {
				imgObj.className = "dhtmlxMenu_SubLevelArea_Item_Icon "
						+ imgSrc
			} else {
				if (String(imgObj.className).search(
						"dhtmlxMenu_TopLevel_Item_Icon") === 0) {
					imgObj.src = this.imagePath + imgSrc
				} else {
					imgObj.style.backgroundImage = "url('" + this.imagePath
							+ imgSrc + "')"
				}
			}
		} else {
			if (levelType == "TopLevel") {
				...
				newImgObj.className = "dhtmlxMenu_" + levelType + "_Item_Icon_"
				...
				this.idPull[this.idPrefix + id].style.paddingLeft = (this._rtl ? this.topLevelItemPaddingIconNotExists
						: this.topLevelItemPaddingIconExists)
						+ "px";
				this.idPull[this.idPrefix + id].style.paddingRight = (this._rtl ? this.topLevelItemPaddingIconExists
				...
				newImgObj.style.backgroundImage = "url('" + this.imagePath
						+ imgSrc + "')";
				newImgObj.id = "image_" + this.idPrefix + id;
				this.idPull[this.idPrefix + id].appendChild(newImgObj)
			}
		...
		if (imgObj != null) {
			...
			if (levelType == "TopLevel") {
				this.idPull[this.idPrefix + id].style.paddingLeft = this.topLevelItemPaddingIconNotExists
						+ "px";
				this.idPull[this.idPrefix + id].style.paddingRight = this.topLevelItemPaddingIconNotExists
						+ "px"
			}
		}
	}
...
dhtmlXMenuObject.prototype.removeItem = function(id) {
	...
	if (this.itemPull[id]["type"] == "separator") {
		this.idPull["separator_" + id].parentNode
				.removeChild(this.idPull["separator_" + id]);
		...
		if (this.itemPull[id]["complex"]) {
			...
			for ( var q = 0; q < items.length; q++) {
				if (this.itemPull[items[q]]["type"] == "separator") {
					...
					;
					this.idPull[items[q]].parentNode
							.removeChild(this.idPull[items[q]]);
			...
			for ( var q = 0; q < polygons.length; q++) {
				...
				if (this._isIE6) {
					var FC = "polygon_" + polygons[q] + "_ie6cover";
					if (this.idPull[FC] != null) {
						document.body.removeChild(this.idPull[FC]);
						delete this.idPull[FC]
					}
				}
				;
	...
	if (this.idPull["polygon_" + parentId] != null) {
		if (this.idPull["polygon_" + parentId].childNodes.length == 0) {
			...
			if (this._isIE6) {
				var FC = "polygon_" + parentId + "_ie6cover";
				if (this.idPull[FC] != null) {
					document.body.removeChild(this.idPull[FC]);
					delete this.idPull[FC]
				}
...
dhtmlXMenuObject.prototype.addContextZone = function(US) {
	var zone = document.getElementById(US);
	var zoneExists = false;
	for ( var a in this.contextZones) {
		zoneExists = zoneExists || (a == US) || (this.contextZones[a] == zone)
	}
	;
	if (zoneExists == true)
		return false;
	this.contextZones[US] = zone;
	var main_self = this;
	if (cU) {
...
dhtmlXMenuObject.prototype.removeContextZone = function(US) {
	if (!this.isContextZone(US))
		return false;
	var zone = this.contextZones[US];
	if (cU) {
		zone.onmouseup = null
	} else {
	...
	;
	delete this.contextZones[US];
	return true
};
dhtmlXMenuObject.prototype.isContextZone = function(US) {
	var isZone = false;
	if (this.contextZones[US] != null) {
		if (this.contextZones[US] == document.getElementById(US))
			isZone = true
	}
	;
	return isZone
...
dhtmlXMenuObject.prototype._showContextMenu = function(x, y, US) {
	if (this.idPull["polygon_" + this.idPrefix + this.topId] == null)
		return false;
	...
	this.callEvent("onContextMenu", [ US ])
};
...
dhtmlXMenuObject.prototype._doOnContextBeforeCall = function(e, cZone) {
	...
	var px = (_isIE || cU || _KHTMLrv ? e.offsetX : e.layerX);
	var py = (_isIE || cU || _KHTMLrv ? e.offsetY : e.layerY);
	var mx = ej(p) + px;
	var my = dg(p) + py;
	if (this.mR("onBeforeContextMenu")) {
...
dhtmlXMenuObject.prototype.getItemPosition = function(id) {
	...
	var obj = (this.idPull["polygon_" + parent] != null ? this.idPull["polygon_"
			+ parent]
			: this.base);
...
dhtmlXMenuObject.prototype.setItemPosition = function(id, pos) {
	...
	;
	var isOnTopLevel = (this.itemPull[id]["parent"] == this.idPrefix
			+ this.topId);
...
dhtmlXMenuObject.prototype.getParentId = function(id) {
	...
	;
	return ((this.itemPull[id]["parent"] != null ? this.itemPull[id]["parent"]
			: this.topId).replace(this.idPrefix, ""))
};
dhtmlXMenuObject.prototype.addNewSibling = function(nextToId, itemId, itemText,
	...
	this._addItemIntoGlobalStrorage(id, parentId, itemText, "item", disabled,
...
dhtmlXMenuObject.prototype.addNewChild = function(parentId, pos, itemId,
		itemText, disabled, imgEnabled, imgDisabled) {
	if (parentId == null) {
		if (this.context) {
			parentId = this.topId
		} else {
			this.addNewSibling(parentId, itemId, itemText, disabled,
					imgEnabled, imgDisabled);
			if (pos != null)
	...
	this._addItemIntoGlobalStrorage(itemId, parentId, itemText, "item",
			disabled, imgEnabled, imgDisabled);
	if (this.idPull["polygon_" + parentId] == null) {
...
dhtmlXMenuObject.prototype._addItemIntoGlobalStrorage = function(itemId,
		itemParentId, itemText, itemType, disabled, img, imgDis) {
	var item = {
		id : itemId,
		title : itemText,
		imgen : (img != null ? img : ""),
...
dhtmlXMenuObject.prototype._renderSublevelPolygon = function(id, parentId) {
	...
	s.className = "dhtmlxMenu_" + this.skin + "_SubLevelArea_Polygon_"
			+ (this._rtl ? "right" : "left");
...
dhtmlXMenuObject.prototype._renderSublevelItem = function(id, pos) {
	...
	;
	if (this.itemPull[id]["complex"]) {
		var j_nodes = "<div class='dhtmlxMenu_SubLevelArea_Item_Arrow " + arw
				+ "' id='arrow_" + this.itemPull[id]["id"] + "'>"
	} else {
		var j_nodes = ""
	}
	;
	if (j_icon.length > 0) {
		...
		if (tp == "checkbox" || tp == "radio")
			j_icon = "<div id='image_" + this.itemPull[id]["id"]
	...
	if (this.itemPull[id]["title"].indexOf('|') >= 0) {
		var tempLink = this.itemPull[id]["title"].split('|');
		k.innerHTML = j_icon + "<a href='" + tempLink[1]
				+ "' onclick=\"document.location.href='" + tempLink[1]
				+ "'\" target='_blank'>" + tempLink[0] + "</a>" + j_nodes;
	} else {
	...
	if (this.itemPull[id]["hotkey"].length > 0 && !this.itemPull[id]["complex"]) {
		var p = "";
		k.innerHTML += "<div class='dhtmlxMenu_SubLevelArea_Item_HotKey'>"
				+ this.itemPull[id]["hotkey"] + "</div>"
	}
	;
	...
	k.onclick = function(e) {
		if (!main_self.mR("onClick") && main_self.itemPull[this.id]["complex"]) {
			return
		}
		;
		e = e || event;
		...
		case "checkbox":
			main_self._checkboxOnClickHandler(this.id.replace(
					main_self.idPrefix, ""), tc + td + "n", cas);
			break;
		case "radio":
			main_self._radioOnClickHandler(this.id.replace(main_self.idPrefix,
					""), tc + td + "n", cas);
			break;
		case "item":
			main_self._doOnClick(this.id.replace(main_self.idPrefix, ""), tc
					+ td + "n", cas);
			break
		}
		;
...
dhtmlXMenuObject.prototype._renderSeparator = function(id, pos) {
	var gR = (this.context ? "SubLevelArea"
			: (this.itemPull[id]["parent"] == this.idPrefix + this.topId ? "TopLevel"
					: "SubLevelArea"));
	if (gR == "topLevel" && this.context)
		return;
	var main_self = this;
	var k = document.createElement("DIV");
	k.id = "separator_" + id;
	k.className = "dhtmlxMenu_" + this.skin + "_" + gR + "_Separator";
	k.onselectstart = function(e) {
		e = e || event;
		e.returnValue = false
	};
	k.onclick = function(e) {
		...
		main_self._doOnClick(this.id.replace("separator_" + main_self.idPrefix,
				""), "--s", cas)
	};
	if (gR == "TopLevel") {
		if (pos != null) {
...
dhtmlXMenuObject.prototype.unload = function() {
	...
	;
	this.setSkin = null;
	var basicMethods = new Array("_enableDacSupport", "_selectedSubItems",
	...
	basicMethods = null;
	var extMethods = new Array("setItemEnabled", "setItemDisabled",
			"isItemEnabled", "_changeItemState", "getItemText", "setItemText",
			"loadFromHTML", "hideItem", "showItem", "isItemHidden",
			"_changeItemVisible", "setUserData", "getUserData", "setOpenMode",
			"setWebModeTimeout", "enableDynamicLoading", "_updateLoaderIcon",
			"getItemImage", "setItemImage", "clearItemImage",
			"setAutoShowMode", "setAutoHideMode", "setContextMenuHideAllMode",
			"getContextMenuHideAllMode", "setVisibleArea", "setTooltip",
			"getTooltip", "setHotKey", "getHotKey", "setItemSelected",
			"setTopText", "setRTL", "setAlign", "setHref", "clearHref",
			"getCircuit", "_clearAllSelectedSubItemsInPolygon",
		...
		items : function(arr, parent) {
			...
			for ( var i = 0; i < arr.length; i++) {
				...
				if (item.type == "separator") {
					...
					this.addNewChild(parent, pos, item.id, item.text,
							item.disabled, item.img, item.img_disabled);
					lastItemId = item.id;
					if (item.items)
