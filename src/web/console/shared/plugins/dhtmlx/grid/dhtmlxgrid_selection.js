...
dhtmlXGridObject.prototype.enableBlockSelection = function(mode) {
	if (typeof this._bs_mode == "undefined") {
		...
		this.obj.onmousedown = function(e) {
			if (self._bs_mode)
				self._OnSelectionStart((e || event), this);
			return true
		};
...
dhtmlXGridObject.prototype._getCellByPos = function(x, y) {
	...
	for ( var i = 0; i < this._cCount; i++) {
		...
		if (x <= 0) {
			while (true) {
				if (_x._childIndexes
						&& _x._childIndexes[i + 1] == _x._childIndexes[i])
...
dhtmlXGridObject.prototype._OnSelectionMove = function(event) {
	...
	if (this._startSelectionCell == null)
		this._endSelectionCell = this._startSelectionCell = this
				.getFirstParentOfType(event.srcElement || event.target, "TD");
		...
		if ((event.srcElement || event.target).className == "dhtmlxGrid_selection")
			...
			var t = this.getFirstParentOfType(event.srcElement || event.target,
					"TD");
			if (t.parentNode.idd)
				this._endSelectionCell = t
		}
	}
	;
...
dhtmlXGridObject.prototype._OnSelectionStop = function(event) {
	...
	if (parseInt(this._selectionObj.style.width) < 2
		...
		var src = this.getFirstParentOfType(event.srcElement || event.target,
				"TD");
...
dhtmlXGridObject.prototype._CreateSelection = function(x, y) {
	if (this._selectionObj == null) {
		...
		this._selectionObj.oncontextmenu = function() {
			return false
		};
...
dhtmlXGridObject.prototype.copyBlockToClipboard = function() {
	if (this._selectionArea != null) {
		...
		for ( var i = this._selectionArea.LeftTopRow; i <= this._selectionArea.RightBottomRow; i++) {
			var data = this._serializeRowToCVS(this.rowsCol[i], null,
	...
	for ( var i = startRow; i < endRow; i++) {
		...
		for ( var j = startCol; j < endCol; j++) {
			...
			if (this._onEditUndoRedo)
				this._onEditUndoRedo(2, row.idd, j, serialized[k][l], ed
						.getValue());
			if (ed.combo) {
