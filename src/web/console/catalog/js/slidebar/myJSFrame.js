...
function $f(){
	...
	if(typeof arguments[0] !="string"){
		...
		elem = arguments[0];
		if(!elem["version"]){
			$f._Method.Element.apply(elem);
			if($f._appendMethod){
				$f._appendMethod.apply(elem);
			}
		}
		return elem;
	}
	...
	if(argID.indexOf(" ")<0 && argID.indexOf(",")<0 && argID.indexOf(".")<0 && argID.indexOf("[")<0 && argID.indexOf(">")<0){
	...
	var path = argID.replace(/(^,*)|(,*$f)/g,"").split(",");
	var allelem = [];
	for(var a=0,b; b=path[a]; a++){
		var pStr = b = path[a].trim().replace(/ +/g," ");
		var pArr = pStr.split(/ |>/);
		for (var i=0,q; q=pArr[i]; i++){
			if (q.indexOf("#")==0){
				if(!document.getElementById(q.substring(1)))return null;
				elem = $f(q.substring(1)); 
				continue;
			}
			var attsel = [];
			var elem_temp=[];
			if (q.indexOf(".")!=-1){
				var tags = q.replace(/\[.*?\]/gi,function($f1){attsel.push($f1.replace(/\[|\]/g,""));return "";});
				var tag = tags.split(".")[0];
				var cn = tags.split(".")[1];
				if (elem == null){
					elem_temp = $f._find(tag,cn,arguments[1] || document);
				}else{
					if (elem instanceof Array){
						var arr = [];
						elem.each(function(obj){$f._find(tag,cn,obj).each(function(){arr.push(arguments[0])});});						
						elem_temp = arr;
					}else{
						elem_temp = $f._find(tag,cn,elem);
					}
				}
				elem = $f._attributeSelector(attsel,elem_temp);
				continue;
			}else{
				var tag = q.replace(/\[.*?\]/gi,function($f1){attsel.push($f1.replace(/\[|\]/g,""));return "";});
				if(elem == null){
					elem_temp = $fA((arguments[1] || document).getElementsByTagName(tag)).each(function(obj){$f(obj)});
				}else{
					if (elem instanceof Array){
						var arr = [];
						elem.each(function(obj){$fA(obj.getElementsByTagName(tag)).each(function(obj){arr.push($f(obj))})});
						elem_temp = arr;
					}else{
						elem_temp = $fA(elem.getElementsByTagName(tag)).each(function(obj){$f(obj)});
					}
				}
				elem = $f._attributeSelector(attsel,elem_temp);
			}
...
$f.Version = "0.2.1.4";
$f._find = function(tag,cn,par){
	var arr = par.getElementsByTagName(tag||"*");
	var elem = [];
	for(var i=0,j; j=arr[i]; i++){
		if((" "+j.className+" ").indexOf(" "+cn+" ")<0)continue;
		elem.push(j);
	}
	return elem;
};
$f._attributeSelector = function(attsel,elem_temp){
...
$f._Method = {
	Element	: function(){
		...
		this.getStyle = function(s){
			var value = this.style[s=="float"?($f.Browse.isIE()?"styleFloat":"cssFloat"):s.camelize()];
			if (!value){
		...
		this.setStyle = function(s){
			...
			for (var i=0,j; j=sList[i]; i++){
				var k = j.split(/:(?!\/\/)/g);
				var key = k[0].trim();
				key=key=="float"?($f.Browse.isIE()?"styleFloat":"cssFloat"):key.camelize();
		...
		this.subTag = function(){return $fA(this.getElementsByTagName(arguments[0])).each(function(n){$f(n);});};
	...
	String : function(){
		this.trim = function(){
			...
			if(typeof(_argument)=="string"){
				return this.replace(_argument == " "?/(^\s*)|(\s*$f)/g : new RegExp("(^"+_argument+"*)|("+_argument+"*$f)","g"),"");
		...
		this.camelize = function(){
			return this.replace(/(-\S)/g,function($f1){return $f1.toUpperCase().substring(1,2)});
		};
	...
	Function : function(){
		this.bind = function() {
  			var __method = this, args = $fA(arguments), object = args.shift();
  			return function() {
    			return __method.apply(object, args.concat($fA(arguments)));
  			}
		};
	},
...
$f._Method.Array.apply(Array.prototype);
$f._Method.String.apply(String.prototype);
$f._Method.Function.apply(Function.prototype);
$f._Method.StringBuffer.apply(StringBuffer.prototype);
$f._Method.NameSpace.apply(NameSpace.prototype);
...
$f.Browse = {
	...
	IEVer : function(){return $f.Browse.isIE() ? parseInt(navigator.userAgent.split(";")[1].trim().split(" ")[1]) : 0;}
};
$f(document);
...
$f.Ajax = {
	...
	get	: function(url,complete,error){ return $f.Ajax.Request(url+(url.indexOf("?")==-1?"?":"&")+Math.random(),"get","",complete,error); },
	post : function(url,para,complete,error){ return $f.Ajax.Request(url,"post",para,complete,error);},
	...
	update : function(url,id){ return $f.Ajax.Request(url,(arguments[2]?"post":"get"),(arguments[2]?arguments[2]:Math.random()),function(x){if("INPUT,SELECT,BUTTON,TEXTAREA".hasSubString($f(id).tagName,",")){$f(id).value=x.responseText;}else{$f(id).innerHTML=x.responseText;}});}
};
$f.Cookies = {
    get : function(n){
...
$f.Form = function(n){
	...
	$f.Form._Method.apply(f);
	return f;
};
$f.Form._Method = function(){
	this.serialize = function(obj){
			...
			for(var i=0,j; j=arr[i]; i++){
				...
				elem[na].push($fE(j.value));
			}
			return $f.Form.serialize(obj,elem);
		};
};
$f.Form.serialize = function(obj){
...
$f.Redirect = function(url,paraStr){
	if(!paraStr){
...
function $fD(str){return decodeURIComponent(str);};
function $fE(str){return encodeURIComponent(str);};
function $fV(id){return $f(id)?($f(id).tagName.hasSubStrInArr(["INPUT","TEXTAREA","SELECT","BUTTON"])?$f(id).value : $f(id).innerHTML):"";};
