...
public class MssqlAccumulator implements Accumulator
{
			...
			JdbcSource jdbcSource)
	{
		try
		{
			QuerySql sql = new StringSql("select sum(datalength(" + fieldName
			...
			JdbcSource jdbcSource)
	{
		try
		{
			QuerySql sql = new StringSql("select count(*) as count_alias from "
					+ tableName + " where " + fieldName + " is null or datalength("
					+ fieldName + ") =0");
			Map<String, Serializable> map = jdbcSource.queryForObject(sql);
			long count = 0;
