...
public class DoRegUser implements JspAction
{
			...
			ServletContext servletContext) throws Exception
	{
		...
		jt
				.executeUpdate(new StringSql(
						"insert into VDB_USERS (USERID,PASSWORD,NAME,SEX,BIRTHDAY,MAIL,MOBILE,PHONE,UNIT,ADDRESS,DETAIL) values (?,?,?,?,?,?,?,?,?,?,?)",
						username, new Md5PasswordEncoder().encodePassword(
								password, username), name, sex, csny, email,
						shouji, lxdh, dw, dz, ms));

		((HttpServletResponse) response).sendRedirect("registerSuccess.htm");
	}
}
