...
public class DoGetPieData extends ServletActionProxy
{
			...
			ServletContext servletContext) throws Exception
	{
		...
		// vdb.mydb.vtl.toolbox.VdbTool tool = new
		// vdb.mydb.vtl.toolbox.VdbTool();
				...
				if (sectionList[i] != "")
				{
					...
					if (isNum)
					{
						sqlStr = "select count(*) as c from "
								+ entity.getTableName() + " where " + fieldName
								+ " between " + sectionse[0] + " and "
								+ sectionse[1];
						if (null != je)
						{
							sqlStr = sqlStr + " and "
					...
					pieData += "<slice title='" + sectionList[i] + "' color='"
							+ colors[i % 9] + "'>"
							+ ((HashMap) list.get(0)).get("c") + "</slice>";
				}
			}
		...
		else
		{
			...
			sqlStr = sqlStr + " group by " + fieldName + " order by c desc";
			if (field.isReference())
			{
				...
				sqlStr = sqlStr + " group by " + fieldB + " order by c desc";
				fieldName = fieldB;
			}
			StringSql sql = new StringSql(sqlStr);
			if (null != je)
			...
			for (int i = 0; i < list.size(); i++)
			{
				...
				if (field.getType().getName().equalsIgnoreCase("enum"))
				{
					Map options = ((VdbEnumType) field.getTypeDriver())
							.getOptions();
					pieData += "<slice title='"
							+ options.get(map.get(fieldName)) + "' color='"
							+ colors[i % 9] + "'>" + map.get("c") + "</slice>";
				}
				else
				{
					pieData += "<slice title='" + map.get(fieldName)
							+ "' color='" + colors[i % 9] + "'>" + map.get("c")
							+ "</slice>";
				}
				remain = remain - Integer.parseInt(map.get("c").toString());
				if (i > 14)
				{
					pieData += "<slice title='其他' " + " color='"
							+ colors[(i + 1) % 9] + "'>" + remain + "</slice>";
					break;
				}
			}
			pieData += "</pie>";
