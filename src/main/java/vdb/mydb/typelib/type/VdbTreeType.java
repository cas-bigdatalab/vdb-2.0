...
import vdb.mydb.typelib.data.VdbTree;
...
public class VdbTreeType extends AbstractFieldDriver<VdbTree>
{
	/**
	 * ��ȡ����ö��ֵ
	 * 
	...
	public VdbTree createData() throws Exception
	{
		VdbTree vd = new VdbTree(getOptions());
		return vd;
	}

	public void jdbcInsert(JdbcRowWriter jdbcRowWriter, VdbTree data)
			throws Exception
	{
		jdbcRowWriter.set(_field.getColumnName(), data.getValue());
	}

	public VdbTree jdbcSelect(JdbcRowReader jdbcRowReader) throws Exception
	{
		VdbTree vd = createData();
		vd.setValue("" + jdbcRowReader.get(_field.getColumnName()));
		return vd;
	}

	public void jdbcUpdate(JdbcRowWriter jdbcRowWriter, VdbTree data,
			VdbTree dataOutOfDate) throws Exception
	{
		jdbcRowWriter.set(_field.getColumnName(), data.getValue());
	}
}